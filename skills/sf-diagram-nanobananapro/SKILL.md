...
### Pattern E: Mermaid-to-Visual Rendering

**Trigger**: `sf-diagram-mermaid` Phase 5.5 delegates here, OR user says "render this Mermaid as an image", "make this diagram visual", or pastes Mermaid code and asks for an image.

This is the **default rendering path** for all Mermaid diagram output. Every diagram generated by `sf-diagram-mermaid` flows through this pattern unless the user explicitly opts out.

**Workflow**:
1. Receive Mermaid diagram code (from `sf-diagram-mermaid` or pasted by user)
2. Parse the Mermaid structure:
   - **Nodes**: Extract names, labels, and any record counts or annotations
   - **Edges**: Extract relationships, labels, and arrow types (lookup vs master-detail, sync vs async)
   - **Layout**: Determine spatial flow (LR = left-to-right, TB = top-to-bottom)
   - **Subgraphs**: Map to visual grouping boxes
3. Build a Nano Banana prompt using this template:

```
Professional Salesforce architecture diagram in the architect.salesforce.com style.
Clean white background, rounded rectangle boxes with dark borders and light translucent fills.

Layout: [LR/TB direction from Mermaid]

Entities:
- [Node 1 name] ([color description based on type])
- [Node 2 name] ([color description based on type])
...

Relationships:
- [Node 1] → [Node 2]: "[edge label]" ([thin arrow for lookup / thick arrow for master-detail])
...

[Subgraph groupings if any]

Include a legend in the bottom-right showing relationship types.
Title: "[Diagram title from Mermaid comments or context]"
```

4. Run prerequisites check
5. Generate at 1K for draft review
6. If the user approves or no feedback needed, generate at 4K for final output
7. Open in macOS Preview

**Mermaid-to-visual color mapping**:

| Mermaid Style | Visual Description |
|---|---|
| `fill:#bae6fd,stroke:#0369a1` (sky blue) | "Sky blue box with dark blue border — standard object" |
| `fill:#fed7aa,stroke:#c2410c` (orange) | "Orange box with dark orange border — custom object" |
| `fill:#a7f3d0,stroke:#047857` (green) | "Green box with dark green border — external object" |
| `fill:#fbcfe8,stroke:#be185d` (pink) | "Pink box with magenta border" |
| `fill:#e8710a` (Agentforce orange) | "Orange box — Agentforce component" |
| `fill:#9334e6` (nonprofit purple) | "Purple box — Nonprofit Cloud component" |
| `-->` (single arrow) | "Thin arrow — lookup relationship" |
| `==>` (thick arrow) | "Thick arrow — master-detail relationship" |
| `-.->` (dashed arrow) | "Dashed arrow — optional or conversion relationship" |

**Sequence diagram conversion**:
- Participants become labeled vertical lanes
- Arrows become labeled horizontal flows between lanes
- `activate`/`deactivate` blocks become shaded processing segments
- `alt`/`else` blocks become branching sections with conditional labels
- `Note over` becomes annotation callouts
...
# Input Mermaid:
# flowchart LR
#   Account["Account<br/>(317)"] --> Contact["Contact<br/>(1204)"]
#   Account ==> Opportunity["Opportunity<br/>(89)"]

# Generated prompt:
gemini --yolo "/generate 'Professional Salesforce ERD diagram, architect.salesforce.com style.
Clean white background, rounded rectangle boxes.
Layout: left to right.
Entities:
- Account (sky blue box, 317 records)
- Contact (sky blue box, 1204 records)
- Opportunity (sky blue box, 89 records)
Relationships:
- Account → Contact: thin arrow labeled LK (lookup)
- Account → Opportunity: thick arrow labeled MD (master-detail)
Legend in bottom-right. Title: Core CRM Data Model'"
```
...
|-------|-------------|-------|
| sf-diagram-mermaid | **Default rendering path** — all Mermaid output flows through Pattern E for visual rendering | Automatic via Phase 5.5, or "render this Mermaid as an image" |
